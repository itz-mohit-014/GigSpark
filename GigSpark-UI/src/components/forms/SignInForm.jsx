    ...
    <div className="max-w-md w-full mx-auto rounded-none max-md:rounded-2xl p-4 md:p-8 shadow-input bg-white dark:bg-black">
      ...
      <form className="my-8" onSubmit={handleSubmit(handleSignIn)}>
        <LabelInputContainer className="mb-4">
          ...
          <Input id="email" placeholder="example@gmail.com" type="email" {...register("email", {required:true})} />
        </LabelInputContainer>
        <LabelInputContainer className="mb-4">
          <Label htmlFor="password">Password</Label>
          <Input id="password" placeholder="••••••••" type={showPassword ? "text"  : "password"} {...register("password", {required:true})} />
          <div onClick={() => setShowPassword(!showPassword)} className="absolute right-1.5 top-5 p-2 px-3 rounded-lg z-10 cursor-pointer text-gray-600">
          { showPassword 
          ? <FaRegEye />
          : <FaRegEyeSlash /> }
          </div>
        </LabelInputContainer>
        <Link to={'#'} className="block text-sm -mt-2 text-right font-bold text-gray-700 hover:text-gray-900">Forget password?</Link>
      ...
      <p className="text-center font-medium text-gray-500 text-sm font-primary">Already have an account? <Link to={"/auth/signup"} className="font-bold text-gray-700 hover:text-gray-900">Sign Up</Link></p>
    </div>
  );
}
